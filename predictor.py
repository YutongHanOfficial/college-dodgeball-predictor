...
def generate_dodgeball_score(expected_points):
    # Dodgeball points are scored 1 at a time per round won
    return generate_poisson(expected_points)
...
class SeasonPredictor:
    def __init__(self, past_csv, current_csv=None, regression_factor=0.25, prior_weight=4):
        self.teams = {}
        self.league_avg_points = 3.0 # Average points scored by one team in a dodgeball match
        ...
        with open(filename, mode='r', encoding='utf-8-sig') as file:
            ...
            for row in reader:
                try:
                    ...
                    # Accepting legacy "home/away" or "team1/team2" for neutral court dodgeball
                    home = row.get("team1", row.get("home", "")).strip()
                    away = row.get("team2", row.get("away", "")).strip()
    ...
    def _build_srs_model(self, games, prefix, iterations=100):
        ...
        league_avg = total_points / (len(games) * 2) if games else 3.0
    ...
    def predict_matchup(self, away_team, home_team, num_simulations=10000):
        ...
        for _ in range(num_simulations):
            score_a = generate_dodgeball_score(exp_pts_a)
            score_h = generate_dodgeball_score(exp_pts_h)
            
            # Dodgeball OT (sudden death round, 1 point)
            if score_a == score_h:
        ...
        return {
            ...
            "avg_score_a": round(a_total_pts / num_simulations, 1),
            "avg_score_h": round(h_total_pts / num_simulations, 1),
            "path": path
        }
    ...
    def get_team_rating_history(self, team_name):
        ...
        while current_dt <= end_dt:
            ...
            if date_str in games_by_date:
                ...
                league_avg = total_points / (len(cumulative_games) * 2) if cumulative_games else 3.0
...
@st.cache_resource
def load_predictor():
    past_file = "dodgeball_games_2025.csv" if os.path.exists("dodgeball_games_2025.csv") else None
    curr_file = "dodgeball_games_2026.csv" if os.path.exists("dodgeball_games_2026.csv") else None
...
st.set_page_config(page_title="College Dodgeball Predictor", page_icon="🎯", layout="wide")
...
st.title("🎯 College Dodgeball Predictor Engine", anchor=False)

if predictor is None:
    st.error("⚠️ No game data found! Please upload `dodgeball_games_2025.csv` or `dodgeball_games_2026.csv` to your repository.")
else:
